  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isSelected && 'border-foreground shadow-lg shadow-black/5',
        isAwarded && 'border-amber-400 bg-gradient-to-l from-amber-50 to-card',
        isRejected && 'border-border opacity-60',
        !isSelected && !isAwarded && !isRejected && 'border-border hover:border-muted-foreground',
        className
      )}
    >
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={cn(
              ...
              isAwarded ? 'bg-amber-100' : 'bg-muted'
            )}>
              {isAwarded ? (
          ...
          <div className={cn(
            ...
            bid.status === 'submitted' && 'bg-muted text-muted-foreground',
            bid.status === 'under_review' && 'bg-purple-100 text-purple-700',
            bid.status === 'rejected' && 'bg-red-100 text-red-700',
            bid.status === 'withdrawn' && 'bg-muted text-muted-foreground',
          )}>
      ...
      <div className="p-4">
        <div className="flex items-baseline justify-between mb-4">
          ...
          {savingsPercentage > 0 && (
            <div className="px-2 py-1 rounded-lg bg-muted text-foreground text-sm font-bold">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="p-3 rounded-xl bg-muted text-center">
            ...
            <p className="text-sm font-bold text-foreground">{bid.leadTimeDays} يوم</p>
            <p className="text-xs text-muted-foreground">مدة التسليم</p>
          </div>

          <div className="p-3 rounded-xl bg-muted text-center">
            <Truck className="w-5 h-5 text-muted-foreground mx-auto mb-1" />
            <p className="text-sm font-bold text-foreground">{bid.incoterm}</p>
            <p className="text-xs text-muted-foreground">شروط التسليم</p>
          </div>
        ...
        {isExpanded && (
          <motion.div
            ...
            className="pt-4 border-t border-border mt-2 space-y-3"
          >
            {bid.paymentTerms && (
              <div>
                <p className="text-xs text-muted-foreground mb-1">شروط الدفع:</p>
                <p className="text-sm text-foreground">{bid.paymentTerms}</p>
              </div>
            )}
            ...
            {bid.deliveryLocation && (
              <div>
                ...
                <p className="text-sm text-foreground">{bid.deliveryLocation}</p>
              </div>
            )}
            ...
            <div className="flex gap-2 pt-2">
              {bid.technicalProposalUrl && (
                <a
                  ...
                  className="flex items-center gap-1 px-3 py-2 bg-muted rounded-lg text-sm text-foreground hover:bg-accent transition-colors"
                >
...
function EvaluationScores({ evaluation }: EvaluationScoresProps) {
  const scores = [
    { label: 'السعر', score: evaluation.priceScore, weight: evaluation.priceWeight, color: 'foreground' },
  ...
  return (
    <div className="p-4 rounded-xl bg-gradient-to-l from-muted to-card border border-border mb-4">
      <div className="flex items-center gap-2 mb-3">
        <BarChart3 className="w-4 h-4 text-muted-foreground" />
        <span className="text-sm font-medium text-foreground">تقييم العرض</span>
        <span className="mr-auto text-lg font-bold text-foreground">
      ...
      <div className="space-y-2">
        {scores.map(({ label, score, weight, color }) => (
          <div key={label} className="flex items-center gap-3">
            ...
            <span className="w-12 text-xs font-medium text-foreground text-left">
              {score} <span className="text-muted-foreground">({weight}%)</span>
            </span>
          </div>
        ))}
      </div>
      ...
      {evaluation.comments && (
        <div className="mt-3 pt-3 border-t border-border">
          <p className="text-xs text-muted-foreground">{evaluation.comments}</p>
        </div>
      )}
    </div>
  );
